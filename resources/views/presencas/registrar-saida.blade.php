                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <div class="mb-6 bg-blue-50 p-4 rounded">
                        <div class="flex items-center">
                            ...
                            <div>
                                <p class="text-sm text-blue-700">Entrada registrada às <strong>{{ $entrada->hora_formatada }}</strong></p>
                                @if ($entrada->responsavel)
                                    <p class="text-xs text-blue-600">Responsável: {{ $entrada->responsavel }}</p>
                                @endif
                                @if ($entrada->observacao)
                                    <p class="text-xs text-blue-600 mt-1">{{ $entrada->observacao }}</p>
                                @endif
                            </div>
                        </div>
                    </div>

                    <form action="{{ route('presencas.salvar-saida', $crianca) }}" method="POST">
                        @csrf

                        <div class="mb-4">
                            <label for="responsavel" class="block text-sm font-medium text-gray-700">Responsável pela retirada</label>
