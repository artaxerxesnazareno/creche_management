                ...
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <div class="mb-6 bg-blue-50 p-4 rounded">
                        <div class="flex items-center">
                            ...
                            <div>
                                <p class="text-sm text-blue-700">Entrada registrada às <strong>{{ $entrada->hora_entrada ? \Carbon\Carbon::parse($entrada->hora_entrada)->format('H:i') : '-' }}</strong></p>
                                @if ($entrada->responsavelEntrada)
                                    <p class="text-xs text-blue-600">Responsável: {{ $entrada->responsavelEntrada->nome }}</p>
                                @endif
                                @if ($entrada->observacoes)
                                    <p class="text-xs text-blue-600 mt-1">{{ $entrada->observacoes }}</p>
                                @endif
                            </div>
                        </div>
                    </div>

                    <form action="{{ route('presenca.salvar-saida', $crianca) }}" method="POST">
                        @csrf

                        <div class="mb-4">
                            <label for="responsavel_saida_id" class="block text-sm font-medium text-gray-700">Responsável pela retirada</label>
                            <select name="responsavel_saida_id" id="responsavel_saida_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" required>
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="observacao" id="observacao" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="Observações sobre a saída (opcional)"></textarea>
                            @error('observacao')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
